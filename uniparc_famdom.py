...
"""
Usage: ./uniparc.py <path2chunk> <outfile>
...
def get_proteome_id(assemblyid):
    ...
    try:
        ...
        proteomeid = None
        with open(f'{path_output}/uniparc/uniparc_log',"a") as f:
            sys.stdout = f
...
def get_fandom(path_output, proteomeid, assemblyid, chunk, index):
    ...
    if not os.path.exists(f"{path_output}/uniparc/{chunk}/"):
        os.makedirs(f"{path_output}/uniparc/{chunk}/")
    try:
        with open(f"{path_output}/uniparc/{chunk}/{assemblyid}.family_and_domain.tsv", 'w') as f:
        ...
        with open(f"{path_output}/uniparc/uniparc_log", 'a') as l:
            sys.stdout = l
            print(f"{proteomeid}: not in UniParc")
            sys.stdout = original_stdout

    return index


if not os.path.exists(f"{path_output}/uniparc"):
    os.makedirs(f"{path_output}/uniparc")
